...
class DistributedClassAwareSampler(Sampler):
    def __init__(self, dataset, num_replicas=None, rank=None, class_sample_path=None):
        ...
        self.class_num_list = [len(self.class_dic[i+1]) for i in range(self.class_num)]
    ...
    def set_epoch(self, epoch):
        ...
        def gen_class_num_indices(class_num_list):
            ...
            id_indices = [self.class_dic[class_indice+1][np.random.permutation(class_num_list[class_indice])[0]] for class_indice in class_indices if class_num_list[class_indice] != 0]
            return id_indices
